...
*/
    ...
    while( $row = $st->fetch() )
    {
        ...
        if( strpos( $title, $q ) !== false )
          $message[ 'movies' ][] = array( 'id_movie' => $row['id_movie'], 'title' => $row['title'], 'director' => $row['director'], 'release_year' => $row['release_year'], 'genre' => $row['genre'], 'cast' => $row['cast'], 'average_rating' => $row['average_rating']);			
    }
